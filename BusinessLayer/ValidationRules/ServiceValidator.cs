...
namespace BusinessLayer.ValidationRules
{
    public class ServiceValidator : AbstractValidator<Service>
    {
        public ServiceValidator()
        {
            ...
            RuleFor(x => x.Icon).MaximumLength(15).WithMessage("İkon en fazla 15 karakterden oluşturulmalıdır");
            RuleFor(x => x.Description).MinimumLength(200).WithMessage("Açıklama en az 200 karakterden oluşturulmalıdır");
            RuleFor(x => x.Icon).MinimumLength(5).WithMessage("İkon en az 5 karakterden oluşturulmalıdır");
        }
    }
}
